...
## AN ITEM CAN BE WRONG FOR A ROOM, AND THE SIM CANNOT SAY SO — parked 2026-08-30, with its falsification test

**The human, on the suitability question:** *"A vending machine in a room is perceptually weird, but
a minibar isn't. One would think having a vending machine in your room would actually be bad, whilst
providing nourishment it would create noise and light."*

**MEASURED, AND THE ANSWER IS NO: nothing in this simulation can harm a need.** `needs.ts:58` states
that *"every number in it is a non-negative safe integer"*; `fitBasisPoints` expresses only HOW WELL
an item serves (`schema.ts:151`); and all four need types carry `capacityTicks`, `refillPerTick` and
`serviceFloorBasisPoints` — **all about serving, none about degrading.** So *"noise and light"* is
not a tuning value, it is **a mechanic that does not exist**.

**WHY IT IS WORTH MORE THAN A FEATURE: it dissolves the enforce-vs-suggest question rather than
answering it.** With it, the game never refuses a placement — **the consequence teaches**, and the
chain runs entirely through systems finished THIS SESSION:

> vending machine in a bedroom -> `night_rest` served worse -> lower review band (G-059's whole-stay
> scorer) -> **the guest says so, by name, in the feed** (G-066a/b).

**That is the Two Point Hospital shape the human named at E-014**: criteria the player can read, and
a mistake they are allowed to make and then see.

**THE FALSIFICATION TEST, so this is a result waiting for a goal rather than a note (§4):** place a
`vending_machine` and a `minibar` in otherwise identical bedrooms, one deterministic run each, and
compare `night_rest`'s band distribution and the review means. **Refuted if the two are identical** —
which they are TODAY, and that identity is the measurement showing the mechanic is absent rather
than weak. *Confirmed the day they differ.*

**Cost, stated honestly**: a negative term in the needs model touches I2's hashed state and the

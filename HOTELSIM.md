...
M4 HARD PREREQUISITE (ADR-0013 §5): the scenario-capital mechanism lands before the first M4 goal starts. --rooms N seeds stock that is cash at the refund rate, and every balance sweep in this project used that flag. Tuning demand and pricing against an inflated opening balance is how a whole milestone's evidence base goes bad quietly.

  BUILT AT G-057, AND THIS PARAGRAPH'S OWN FIGURE WAS WRONG. It read "--rooms 3 carries 375,000p against a 500,000p starting constant" and called that "a 75%-inflated opening balance". Both numbers are the arithmetic of THREE BEDROOMS; the default invocation seeds NINE ROOMS, because --amenities defaults to 1 and seeds one of EACH amenity room type, of which the shipped content has three, each scrapping for the same 125,000p as a bedroom. The seeded stock is (rooms + 3 x amenities) x 125,000p. RE-MEASURED ON THE TREE AT G-057, exact deterministic integers from sim:run --json at --days 5 --seed 42, 3 repeats and 4 seeds agreeing, no aggregation, win32/12cpu quiet: the DEFAULT arm is 150% and not 75%, --rooms 6 --amenities 5 is 525%, and the --rooms 60 bench arm is 1,575%. The 75% reading is real and belongs to --rooms 3 --amenities 0, an arm no sweep has ever run. The error is not drift — nothing that moved at G-040 or G-041 touches it — it is a count taken over one of two populations, and it stood for two milestones because nobody re-derived it.

  WHAT G-057 LANDED, AND WHAT IT DID NOT. A scenario declares openingCapitalPence in packages/content/data/scenarios.json; createWorld reads it once and books the single startingCapital transaction; startingCapitalPence is gone from economy.json and from economySchema, because the economy is the house rules and an opening balance is the situation. The seededStock policy declares what a room the HOST places free does to that number, and BOTH branches are built and tested — but the SHIPPED value is supplementsCapital, which is what every build before G-057 did. Flipping it to drawnFromCapital was measured first and moves 35 tests in 9 files, four of them PINNED EXIT CRITERIA that go vacuous because a 60-room seeded hotel opens 7,375,000p in the red and can then build nothing. A single global capital cannot serve both a bare-plot scenario and a 60-room bench arm; serving both needs a scenario the harness SELECTS, which is PARKING.md's C1 and is ruled to M6. SO THE MECHANISM IS THE PREREQUISITE AND IT IS DISCHARGED; THE FLIP AND THE RE-TAKE ARE M4'S FIRST GOAL.

M5 — **OPENED 2026-08-28 on the human’s M4 sign-off** (`GOALS.md`’s "M4 exit" block carries it, and the ordered queue with it). Render. Pixi ISOMETRIC FLOORPLAN view (~~cross-section~~ — STRUCK 2026-08-26 by G-053b, ADR-0089 §7(a); the word was reversed by ADR-0046 on 2026-08-16 and survived here for ten days and four goals), camera, build tools, HUD, speed controls, save/load UI. The first playable build ships PLACEHOLDER ART — flat coloured shapes with clear silhouettes — and real art is a separate track that replaces them without touching the simulation (ADR-0014, decided 2026-08-08 so M5 neither relitigates it nor waits on it).

  WHY THE STRIKE IS RECORDED IN PLACE RATHER THAN SWEPT AWAY, AND WHY IT IS THE FOURTH COPY. §1's banner states the general form: A RULING IS NOT LANDED UNTIL EVERY COPY OF THE SENTENCE IT REVERSES IS DEAD. ADR-0046 corrected §1 on the day; CLAUDE.md's short form was corrected seven days later, at G-053a; THIS ROW — the milestone plan for the render layer, in the source of truth — was corrected at G-053b, and nobody had run the grep. THREE MORE LIVE COPIES SURVIVE THIS GOAL DELIBERATELY AND ARE NAMED SO THE SILENCE IS NOT READ AS COVERAGE: `.claude/agents/render-engineer.md` lines 3, 37 and 43, which carry the reversed sentence VERBATIM, parenthetical and all, in the standing instructions of the agent that would build M5; and `apps/game/src/scenario.ts:36-37`. Both are outside a sim goal's domain — the first is agent configuration — so they are BLOCKING OBLIGATIONS ON M5 rather than edits made here (GOALS.md's M5 block, PARKING.md, ADR-0089 §7(a)). THE TEST IS ONE COMMAND: `git grep -in "cross-section\|SimTower\|Project Highrise\|side-on"` — every hit is either history, or `tools/viewer/viewer.js:306` which is TRUE because the replay viewer really does draw side-on (ESCALATIONS.md:1304), or an orphan.

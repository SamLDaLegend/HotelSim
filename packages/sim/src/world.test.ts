...
describe('state hash covers the whole world', () => {
  it('changes when an entity is added, so the store cannot hide from I2', () => {
    ...
    const spawn: Command = { kind: 'spawnEntity', entityKind: 'alpha', at: { floor: 0, column: 0, row: 0 } };
  ...
  it('projects the world to JSON without dropping or reordering a field', () => {
    ...
    const world = stepTick(createWorld(6, content), content, [{ kind: 'spawnEntity', entityKind: 'alpha', at: { floor: 0, column: 0, row: 0 } }]);

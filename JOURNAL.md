...
- **State**: save **v25** · summary **v4** · I2 `19de1252e3c9948e` · measure golden
  `4d6050fd9a1ad339` *(**both moved at G-059 and that move IS BEHAVIOUR**: the review scorer changed
...
## WATCH #36 — 2026-08-29 — A GUEST STANDS IN A DOORWAY, AND E-012's SECOND SENTENCE IS ANSWERED

**Instrument**: `apps/game`'s recorder, seed 7, the WATCH surface. Paired arms one commit apart, the
SAME guest, the SAME ticks. Frames under `apps/game/.tmp/g046/` (gitignored, kept so the census can
be re-run). **Verified by the orchestrator out of the SVG bytes, not taken on the builder's report.**

Guest 1, `standard_room` 1 on floor 0:

| tick | BEFORE (HEAD) | AFTER |
|---|---|---|
| 111 | `translate(544.00 260.00)` — **already inside**, since t108 | `translate(480.00 228.00)` — **standing in the doorway**, a declared corridor cell |
| 112 | `translate(544.00 260.00)` — still inside | `translate(544.00 260.00)` — inside |

**Before**, the guest stepped off the stairhead at t107 `0:1:0` and was at `0:1:1` by t108 —
**straight through the room's back edge.** **After**, it goes t110 `0:1:0` -> t111 `0:0:1` (the
doorway) -> t112 `0:1:1`. **380 such doorway entries in 2,880 ticks on the WATCH surface.**

**E-012's second sentence — *"they also seem to jump through walls rather than looking for a
door"* — is answered for ENTERING and is NOT answered for LEAVING.** That split is the finding.

### THE CENSUS, AND THE RESIDUE IS NAMED CLASS BY CLASS

The instrument reproduces WATCH #29 **to the digit** on the before arm (1,554 / 898 / 279 / 377),
which is what makes the after arm comparable rather than merely different.

| | before | after |
|---|---|---|
| through-wall class (WATCH #29's definition) | **309** | **267** — 19.9% -> 14.6% of moves |
| … circulation -> room (**entering**) | 64 | **19** (−70%) |
| … room -> circulation (**leaving**) | 245 | 248 |
| … circulation -> circulation, blocked | 68 | 111 |

1. **248 leaving.** (b) is an APPROACH rule and says nothing about exits — a different mechanism,
   because it depends on where the guest IS rather than where it is going. **Seam offered and taken:
   G-046b.**
2. **19 entering**, down 70%. What remains is `stepTowards`' fallback plus corner-clipping between
   the doorway and a destination deeper inside the room.
3. **111 corridor-to-corridor**, risen with 18% more motion. **Checked rather than assumed: all 111
   (and all 68 before) span a rectangle containing at least one room**, so none is a back-of-house
   artefact.

**A SECOND INSTRUMENT DISAGREES, AND THE DISAGREEMENT IS THE INFORMATION.** The landing census
(`travel.walls.report.test.ts`) is FLAT — 96->95, 0->0, 56->56, 0->0, 0->0 — on 16-35% more motion.
**The door changes where a journey ENDS, not what `stepTowards` does when it is boxed in.** Both
readings are true and they answer different questions; the file's header now carries the table.

### WATCH #29's LABEL IS WRONG AND THE COUNTS ARE RIGHT

The three counts 245 / 68 / 64 reproduce exactly. **The label on the 68 does not**: WATCH #29 reads
*"from its own room to circulation"* and the 68 are moves with **circulation at BOTH ends** whose
route was blocked in the middle. (The 245 is *"origin inside ANY room -> circulation"*, and the
guest's own-lodging split inside it — 178 / 67 — has never been recorded by anybody.) **Counts
stand; one label is corrected here rather than in place, because WATCH #29 is history.**

### WHAT THE ORCHESTRATOR'S BRIEF GOT WRONG

It presented two `stepTowards` defects as *"things the original block did not name"*. **True of the
block, FALSE of the code**: both are documented at the point of use with their rationale, and defect
1 (the landing-only wall test) was **refused on a measurement** — the per-cell rule was built and
made this surface WORSE, 23 through-wall landings becoming 43 where landing-choice took 23 to 6.
**A brief that calls a measured decision an oversight invites a builder to undo it.** Same family as
the G-046 duplicate-block error earlier the same day: *read the artefact before characterising it.*

## WATCH #37 — 2026-08-29 — A GUEST LEAVES THROUGH ITS DOOR, AND ONE CLASS GOT WORSE

**Instrument**: `apps/game`'s recorder, seed 7, paired arms one change apart, same guest, same ticks.
Frames under `apps/game/.tmp/g046b/{before,after}/` (gitignored). **Verified by the orchestrator out
of the SVG bytes.** Guest 1, `games_room` 37 on floor −1:

| tick | BEFORE | AFTER |
|---|---|---|
| 108 | `translate(224.00 109.75)` — inside, `-1:0:1` | **identical** |
| 109 | `translate(328.00 109.75)` — **already out at `-1:1:0`**, straight off the corner | `translate(276.00 135.75)` — **standing in the doorway `-1:1:1`** |
| 110 | gone from the floor | `translate(328.00 109.75)` — the spine, one tick later |

**318 doorway EXITS in 2,880 ticks, against 6 before** — the mirror of WATCH #36's 380 entries.

### THE CENSUS, AND THE HONEST HALF IS THAT ONE CLASS GOT WORSE

| | before | after |
|---|---|---|
| move events | 1,832 | 1,966 (+7.3%) |
| **blocked, all classes** | **378** | **191 (−49%)** |
| **through-wall class** | **267** | **119 (6.1% of moves, from 14.6%)** |
| … room -> circulation (**leaving**) | **248** | **49 (−80%)** |
| … circulation -> room (**entering**) | 19 | **70 (x3.7 — WORSE)** |
| corridor -> corridor, blocked *(a FOURTH class, not part of the 267)* | 111 | 72 (−35%) |

**THE WORSENED CLASS IS NAMED WITH ITS MECHANISM AND MEASURED ON BOTH ARMS.** All 70 are
`stepTowards`' **fallback**, all 70 **single-candidate** (`leastOnColumn === mostOnColumn`, so *"every
option was a wall"* is really *"there was one option"*), all 70 **same-row**, and all 70 start **on a
doorway**. On the before arm **19 of 19** are the same four things. **So it is the same population
getting more occasions, not a second cause** — the reading G-058 established for this residue.

**The occasion is a PARITY**: the shipped plate puts a lane every **2** columns and
`guestCellsPerTick` is **3**, so a guest stepping three columns *from a lane* lands *on a room* where
the same guest stepping three columns *from a room* lands *on a lane*. **The exit rule puts guests on
lanes at the start of a journey.** That is the fallback, which this goal put out of scope, **so it is
reported with its mechanism rather than repaired.** *Falsification test, parked: re-run the census at
`guestCellsPerTick` 2 or 4 — the 70 should collapse while the leaving class holds at 49.*

**Net: the through-wall class more than halves and blocked moves fall by half.** The class the human
named — *"guests walking out through walls looks worse"* — is down 80%.

### TWO GUARDS, AND THE LIVELOCK WAS REPRODUCED BEFORE IT WAS PREVENTED

A naive exit rule cycles: the doorway sends the guest out, every candidate beyond is a wall, the
fallback pushes it back in, the rule fires again. **Two cells that map onto each other, neither wrong
on its own tick.** Guard 1 refuses when the doorway is beyond one tick's budget — which makes
`stepTowards` single-candidate ON the doorway, so the fallback **provably cannot run** and the rule
cannot fire twice running. Guard 2 refuses a doorway that is further from the leg than the guest is.
**Termination is a proof, not a hope**, and each guard is mutation-probed: deleting guard 2 turns 4
tests red, guard 1 turns 2 red, restored and sha-verified.

**The residue is scope, stated as scope**: a guest whose door is BEHIND it still leaves through the
wall in front, because going out of the far door needs a route AROUND the room — option (c), refused
twice on cost.

### WHAT I GOT WRONG IN WATCH #36, FOUND BY THE BUILDER

**"267 of 1,832 — 248 leaving / 19 entering / 111 corridor-to-corridor" READS AS A PARTITION OF 267
AND IS NOT ONE.** 248 + 19 = 267; the 111 is a **fourth class outside the through-wall total**
(`blocked` = 378 = 267 + 111). **WATCH #36's table put all four under one indent, and that is what
propagated into G-046b's brief.** Counts stand; the arithmetic is stated here and the table above is
indented to show it.

**And the instrument had a real bug**: `census.ts`'s `throughWallClass` field is
`fromOtherRoomOut + intoRoom + roomToRoom` and **omits `fromOwnRoomOut`**, so it printed **209** where
WATCH #36 reported 267. **The reported figure was right and had been assembled by hand; the FIELD was
wrong.** Fixed in the instrument. *A hand-assembled number outliving a wrong automated one is luck,
not method.*
...
## WATCH #38 — 2026-08-30 — PRE-FLIGHT BEFORE THE STRANGER. The ladder finally tells the player what to build, and E-013 turns out to be a function of WIDTH.

**Nobody had run the game since G-066b**, and four goals changed behaviour and content since: G-046
and G-046b (doors both ways), G-060 (the ladder), G-068 (the purse). **This is a pre-flight for
G-067, not a goal's WATCH** — it exists so a stranger does not meet a defect nobody looked for.

**Instrument**: the shipped game at `http://localhost:5180`, `pnpm --filter @hotelsim/game build`
green in 9.73s, **no console errors**, tick 1,095, 143fps.

### THE FINDING THAT MATTERS, AND IT DISCHARGES A COMPLAINT OPEN SINCE G-051b

The HUD, read verbatim:

```
cash £10,000.00
stars ★★★★☆ 4 of 5 · Four Star · what the hotel HAS, not what its guests said
earned by 12 Standard Room · 2 sets of Games Room/Cafe/Lounge · 1 kind of Conference Hall/Spa/Theatre
next 5 stars: 24 Standard Room — has 18 · 3 sets of Games Room/Cafe/Lounge — has 2
```

> **G-051b's standing complaint was: *"the correct play under the shipped tables is now 'never take
> the fifth star unless you also add a third amenity set', worth +48,591,500p over a year, AND
> NOTHING IN THE GAME SAYS SO."*** **The game now says so**, in the one line a player reads to decide
> what to build next — **and it says it in the ladder's own vocabulary (`sets`), not as advice
> bolted on.**

**That is not a side effect of G-060; it is the point of it**, and it was only visible by running the
thing. `earned by` also now reads **`2 sets of`** rather than `3 kinds of`, so the counting mode
change reached the screen. **`cash £10,000.00`** confirms G-068's raised purse is live.

### WATCH #35's CLIPPING FINDING IS WITHDRAWN, AND THE CAUSE IS INSTRUCTIVE

WATCH #35 recorded: *"HUD rows CLIP rather than wrap — `rooms 9/9 valic`, `Games Room £2,50`."*
**Measured now at the same 404px viewport: ZERO elements with `scrollWidth > clientWidth`, and
`documentElement.scrollWidth` does not exceed the viewport.** It does not reproduce.

**The likely cause is the instrument, not the page.** That reading was taken immediately after
resetting an emulated **1440×900** viewport back to the pane's own size. **What was read as text
clipping was almost certainly a 1440px-wide layout cropped into a 404px image.** *A property of the
instrument attributed to the subject* — the same shape as G-064's *"the tool's context window was the
population and I read it as `buildRoom`'s"*, and CLAUDE.md rule 4's REGIME slot in a perceptual
register rather than a timing one.

**The finding is withdrawn rather than restated** (rule 5). It also stands in `eb04fa1`'s commit
message, which cannot be edited; **this entry is the correction of record.**

### E-013 IS A FUNCTION OF WIDTH, WHICH REPLACES TWO INCOMPARABLE READINGS WITH A MECHANISM

| viewport | HUD height | stage height | stage share |
|---|---|---|---|
| **1440 x 900** (a real window) | 112px | **605px** | **67.2%** |
| **404 x 419** (this pane) | **368px** | **0px** | **ZERO** |

**THE MECHANISM: narrow width -> the HUD wraps -> the HUD's `auto` row grows -> the `1fr` stage row
collapses.** At 404px the HUD alone needs 368 of 419 pixels and **the picture is literally zero
pixels tall.**

**This supersedes both prior readings rather than joining them**: WATCH #33's 55% at 580px and WATCH
#35's 36.1% at 404px were **two points on a curve nobody had identified**, quoted as if they were the
same measurement getting worse. **They were different widths.** *That is exactly what rule 4's REGIME
slot exists to prevent, and both readings were mine or inherited without it.*

**At a real desktop size the game is fine.** E-013 remains open and remains real — a window this
small shows no game and cannot scroll to reach the controls — but its severity is now stated as a
threshold rather than a vibe.

### CONSEQUENCE FOR G-067, AND IT GOES IN THE PROTOCOL

**The stranger must play in a normally-sized desktop window**, and that instruction is now part of

...
## ADR-0109 — LOSING IS TOLD, NOT ENFORCED. The lose state is a warning surface, and its third fact is how long RECOVERY remains possible.

**RULED 2026-08-30 BY THE HUMAN**, two answers in one sitting.

1. **The lose state WARNS and changes nothing mechanically.** Rejected: an insolvency state with
   teeth (no building, arrivals dry up, staff leave), and warn-then-bite.
2. **`floorConstructionCostPence` 750,001 -> 1,000,000**, the round `4 x cheapestRoom`, closing E-016.

### Why warn-only is the answer the EVIDENCE was already pointing at

**Both of this session's economy goals found the same defect, from opposite directions, and neither
found a missing mechanic:**

- **G-068**: a hotel seeded at tier 5's scale and service buys both facilities out of the purse,
  reads **five stars with an empty shortfall**, earns nothing, and closes **79,026,000p in the red**
  over 1,000 days. **It looks successful the whole way down.**
- **G-069**: opening a storey takes **~99% of the till in a single click** — 1,000,001p of 1,011,500p
  — and **nothing warns first.**
- **G-051b, still standing until WATCH #38**: *"the correct play is X, AND NOTHING IN THE GAME SAYS
  SO."*

> **In every case the defect is that NOTHING TELLS THE PLAYER.** A consequence would have added a
> second thing the game does not explain, on top of the first. **The ruling matches the measurements
> rather than the genre.**

**And it is consistent with ADR-0108**: bankruptcy is RECOVERABLE, so a state that blocks building
would fight the ruling that says you can climb back.

### THE THREE FACTS, AND THE THIRD IS DERIVED

The human's own preview is the specification:

```
cash −£2,340.00  ·  losing £410 a night  ·  4 nights to nothing
```

**Same shape as G-062's rating cell** — *what am I* / *why am I that* / *what next* — which the human
ruled to be the core of that system.

- **CASH** — `balanceOf(ledger)`, which already exists and is folded, never stored (I4).
- **THE BURN** — **last night's net settlement**, not a rolling average. *A window would be a number
  nobody can source (§2.1); "a night" is the unit the settlement already runs in, so last night's net
  needs no chosen constant.*
- **THE RUNWAY — AND THE PREVIEW'S ARITHMETIC DOES NOT CLOSE AS WRITTEN, WHICH IS INFORMATIVE.** It
  shows **negative cash with four nights remaining**. That is only coherent if *"nothing"* is
  measured against something OTHER than cash.

> **RULED HERE, from the mechanism rather than invented: the runway is measured against
> `balance + liquidationValue`, which is `canDrawLoan`'s OWN gate quantity.** It is *how much hotel
> is left to sell before you cannot come back* — and since ADR-0108 makes bankruptcy recoverable,
> **the third fact tells the player HOW LONG RECOVERY REMAINS POSSIBLE.** That is a thing they can
> act on, and it borrows an existing mechanism rather than adding one.

**A NEGATIVE BALANCE IS THEREFORE NOT THE TRIGGER**, and that is the interesting consequence: a hotel
can be deep in debt with plenty of hotel left to sell, and a hotel can be in credit and one night
from the end of its options. **The warning is about the runway, not the sign of the balance.**

### What this obliges

- **It is a RENDER goal with a sim-side selector.** The burn and the runway are derived from the
  ledger and from liquidation value; **nothing is stored, so no save bump** — the pattern G-051a's
  star rating and G-066a's ring both used.
- **It must not fire when it is noise.** A hotel that is profitable has no runway to show. The
  visibility rule is part of the goal and is measured, not asserted.
- **G-067's fifth question SURVIVES and is narrowed.** It no longer asks *what should losing be*;
  it asks **whether this warning is legible, and whether it arrives in time to act on.** That is a
